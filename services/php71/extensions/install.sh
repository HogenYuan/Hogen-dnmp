...
installExtensionFromTarGz()
{
    tgzName=$1
    extensionName="${tgzName%%-*}"

    mkdir ${extensionName}
    tar -xf ${tgzName}.tar.gz -C ${extensionName} --strip-components=1
    ( cd ${extensionName} && phpize && ./configure && make ${MC} && make install )
...
installChilkat()
{
    ...
    tar -xf ${tgzName}.tar.gz -C ${extensionName} --strip-components=1
    ##需要修改cp后的路径，版本不同路径不同
    cd ${extensionName} && cp chilkat_9_5_0.so /usr/local/lib/php/extensions/no-debug-non-zts-20160303
    cd /usr/local/lib/php/extensions/no-debug-non-zts-20160303 && chmod +x chilkat_9_5_0.so

    docker-php-ext-enable chilkat_9_5_0.so
}
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    installExtensionFromTgz swoole-4.3.0

fi

if [[ -z "${EXTENSIONS##*,chilkat,*}" ]]; then
    echo "---------- Install chilkat ----------"
    isPhpVersionGreaterOrEqual 7 0

    installChilkat chilkat-9.5.0
fi
